...
static void setlayouttoggle(const Arg *arg);
...
/* scratchpad */
# define SPTAGMASK (1u << sizeof tags / sizeof * tags)
...
focus(Client *c)
{
	if (!c || !ISVISIBLE(c))
		...
		for (c = selmon->stack; c && (!ISVISIBLE(c) || (c->issticky && !selmon->sel->issticky)); c = c->snext);
	    	//for (c = selmon->stack; c && (!ISVISIBLE(c) || (c->isfloating && c->tags == ~0)); c = c->next);
...
static void scratchpad_show ()
{
	...
	{
		if (scratchpad_last_showed -> tags != SPTAGMASK)
		{
			scratchpad_last_showed -> tags = SPTAGMASK;
			focus(NULL);
			arrange(selmon);
		}
		else
...
updatedocks(Monitor *m) {
	...
	for(c = m->clients; c; c = c->next) {
		...
		// a dirty but I just want to force bottom
		// positioning
		c->isdock = 3;
